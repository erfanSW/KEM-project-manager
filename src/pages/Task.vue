<template>
  <div>
    <q-btn icon="add" label="اضافه کردن ستون" color="indigo-5" class="q-ml-md" @click="open_status_modal" outline/>
    <q-page class="flex">
      <div class="row inline q-mt-md" v-for="(st,i) in statuses" :key="i">
        <div class="col-lg-2 col-md-4 col-sm-6">
          <div>
            <banner @click="open_newTask" :name="st.name" :color="st.color" :status="st.id" class="q-ml-md">
            </banner>
            <task-card class="taskcard q-ml-md" v-for="task in st.tasks" :key="task.id" :task="task" :color="st.color"
                       @click.native="taskcardaction(task)"></task-card>
          </div>
        </div>
      </div>
      <new-task :project="project_id"></new-task>
      <view-task :task="viewTask" :project="project_id"></view-task>
      <new-status :project="project_id"></new-status>
    </q-page>
  </div>
</template>
